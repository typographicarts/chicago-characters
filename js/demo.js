 ...
 */
{
    ...
    class Grid {
		...
		initEvents() {
			...
			for (let item of this.items) {
				...
				let href = item.DOM.el.querySelector('h3').innerHTML.replace(/[\W_]+/g, '-');
				item.DOM.el.setAttribute('href', '#'+href);
			}

			// Close item.
			this.DOM.closeCtrl.addEventListener('click', () => {
				//this.closeItem();
			...
			this.hashchangeFn = (e) => {
				...
				var el = document.querySelector('a[href="#'+hash+'"].grid__item');
				
				if(el == null && this.current != -1){
					this.closeItem();
				}else if(this.current == -1){
					for(let i=0; i<this.items.length; i++){
						let item = this.items[i];
						if(item.DOM.el.getAttribute('href') == '#'+hash){
							this.openItem(item);
							break;
						}
					}
		...
		openItem(item) {
			...
			TweenMax.to('main > header, main > footer', 1.2, {
				ease: Expo.easeInOut,
				delay: 0.55,
				autoAlpha: 0,
			});
		...
		closeItem(withAnimation = true) {
			...
			TweenMax.to('main > header, main > footer', withAnimation ? 1.2 : 0, {
				ease: Expo.easeInOut,
				delay: 0.15,
				autoAlpha: 1,
			});
	...
	imagesLoaded(document.querySelectorAll('.grid__item-img'), () => {
		...
		var indicator = document.querySelector('.content__indicator');
		
		if(indicator != null){
			indicator.addEventListener('click', function(){
				var vh = Math.max(document.documentElement.clientHeight, window.innerHeight || 0);
				var indicator = indicator.getBoundingClientRect();
				var scroll = window.pageYOffset || document.documentElement.scrollTop;
				var data = {scroll:0}
				var dur = Math.max(0.35, (indicator.bottom / 1000) * 1.1);
			
				TweenMax.to(data, dur, {scroll:1, ease:Power2.easeInOut, onUpdate:function(){
					window.scrollTo(0, scroll + (indicator.bottom * data.scroll));
				}})
			});
		}
		
		var grid_bgs = document.querySelectorAll('.content__item');
		for(var i=0; i<grid_bgs.length; i++){
			var newdiv = document.createElement('div');
			newdiv.classList.add('content__item-credits');
			newdiv.innerHTML = '<a href="http://flydesign.com"><img src="img/flydesign.png" alt="FlyDesign"></a>';
			grid_bgs[i].appendChild(newdiv);
		}
